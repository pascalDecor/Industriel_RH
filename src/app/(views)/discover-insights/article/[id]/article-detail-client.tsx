...
export default function ArticleDetailClient() {
  ...
  const handleShare = {
    ...
    email: () => {
      const subject = `Article int├®ressant: ${shareTitle}`;
      const body = `Je pensais que cet article pourrait vous int├®resser:\n\n${shareTitle}\n\n${shareUrl}`;
  ...
  const fetchArticle = async (id: string) => {
    try {
      ...
      // R├®cup├®rer les articles similaires apr├¿s avoir charg├® l'article principal
      if (data) {
  ...
  const fetchRelatedArticles = async (currentArticle: ArticleData) => {
    try {
      ...
      // Construire la requ├¬te pour les articles similaires bas├®e sur les tags et sp├®cialit├®s
      ...
      if (response.ok) {
        ...
        // En cas d'erreur, r├®cup├®rer les articles les plus r├®cents
      ...
      // Essayer de r├®cup├®rer quelques articles r├®cents en fallback
      try {
        const fallbackResponse = await fetch('/api/articles?published=true&limit=4&sortBy=createdAt&sortOrder=desc');
        if (fallbackResponse.ok) {
          const fallbackData = await fallbackResponse.json();
  ...
  if (!article) {
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
        <div className="text-center max-w-md w-full">
          <h1 className="text-xl md:text-2xl font-bold text-gray-800 mb-4">Article non trouv├®</h1>
          <Button
            variant="primary"
            onClick={() => router.push('/discover-insights')}
            className="rounded-full w-full sm:w-auto"
