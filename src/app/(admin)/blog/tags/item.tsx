...
export default function ItemTags({ tag, isEnglishView, onChange }: { tag: Tag, isEnglishView: boolean, onChange: (state: any) => void }) {
    ...
    const handleConfirmDelete = async () => {
        ...
        try {
            await HttpService.delete<unknown>({ url: `/tags/${tag.id}` });
            onChange(true);
        } catch (e) {
    ...
    const label = isEnglishView ? (tag.libelle_en || tag.libelle) : tag.libelle;
    ...
    return (
        <Card className="p-5 border-none mb-3 shadow-none flex flex-row justify-between items-center" key={tag.id}>
            ...
            <ConfirmDeleteModal
                ...
                description={
                    count > 0
                        ? <>Le tag « {label} » est utilisé par {count} article(s). Supprimer quand même ?</>
