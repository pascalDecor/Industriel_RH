                    ...
                    <Dialog open={open} onOpenChange={setOpen}>
                        ...
                        <DialogContent>
                            <AddFonctions sectorId={sectorId} fonction={Fonction.fromJSON(({} as any))} onChange={state => {
                                if (state) {
                                    // Après ajout/mise à jour, on réinitialise la recherche pour voir la liste complète
                                    setSearch('');
            ...
            <div>
                <AsyncBuilder promise={async () => {
                    return HttpService.index<Fonction>({
                        url: '/fonctions?page=' + page + '&search=' + search + '&sectorId=' + sectorId + '&_t=' + changeCount,
                        ...
                        return <>
                            ...
                            {data.data.map((s: any, index: number) =>
                                <ItemFonctions
                                    ...
                                    onChange={(state) => {
                                        if (state) {
                                            // Après suppression/mise à jour, on réinitialise aussi la recherche
                                            setSearch('');
                                            setchangeCount(c => c + 1);
                                            setOpen(false);
                                        }
