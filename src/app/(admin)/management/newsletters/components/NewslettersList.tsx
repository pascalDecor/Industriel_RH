...
export default function NewslettersList({ isVisible }: NewslettersListProps) {
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      confirm(
        `Êtes-vous sûr de vouloir supprimer ${selectedEmails.length} abonnement(s) ?`
      )
    ) {
      try {
  ...
  return (
    <div className="space-y-4">
      <div
        className={`flex items-center justify-end space-x-2 transition-all duration-500 delay-300 ease-out transform ${
          isVisible
      ...
      <div
        className={`grid grid-cols-1 gap-4 transition-all duration-700 delay-400 ease-out transform ${
        ...
        <AsyncBuilder
          ...
          emptyComponent={
            <div className="text-center bg-white rounded-lg p-10 shadow-sm border animate-in fade-in-0 zoom-in-95 duration-500">
              <div className="text-slate-400 text-6xl mb-4 animate-bounce">
                📧
              </div>
                            ...
                            >
                              <td className="px-6 py-4 whitespace-nowrap">
                                <input
                                  ...
                                  onChange={() =>
                                    toggleEmailSelection(newsletter.email)
                                  }
                                  className="rounded border-gray-300 text-blue-600 focus:ring-blue-500"
                                />
                              </td>
                                ...
                                <div className="flex flex-wrap gap-1">
                                  ...
                                  {newsletter.specialites.length > 2 && (
                                    <span
                                      ...
                                      title={newsletter.specialites
                                        .slice(2)
                                        .map((spec) => spec.libelle)
                                        .join(", ")}
                                    >
                                      +{newsletter.specialites.length - 2}
                                    </span>
                              ...
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {newsletter.subscribedAt
                                  ? formatDate(
                                      newsletter.subscribedAt.toString()
                                    )
                                  : "Non défini"}
                              </td>
                              <td className="px-6 py-4 whitespace-nowrap">
